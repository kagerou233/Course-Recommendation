...
export class GraphEventHandler {
  ...
  nodeDblClicked(d) {
    console.log("双击节点触发:", d);
    console.log("getNodeNeighbours方法:", this.getNodeNeighbours);

    if (!this.getNodeNeighbours) {
      console.error("getNodeNeighbours方法未定义！无法展开节点");
      return;
    }
    ...
    const currentNeighbourIds = this.graph.findNodeNeighbourIds(d.id);

    console.log("当前邻居节点IDs:", currentNeighbourIds);

    this.getNodeNeighbours(
      d,
      currentNeighbourIds,
      function ({ nodes, relationships }) {
        console.log("获取到的新节点:", nodes);
        console.log("获取到的新关系:", relationships);
